...
public class YuntuDemo {
     ...
     * @throws IOException        文档显示异常。
     * @throws URISyntaxException 网络访问异常。
     */
    ...
    private static String yuntuTransToPDF(String url) throws YuntuException, IOException,
            URISyntaxException {
        String pdfURL = null;
        // 转换成pdf
        Converter converter = new com.jingyue.DocConversion.Converter();
        converter.setConfig(new YuntuConfig("html"));
        YuntuDoc doc = converter.convert(url);
        if (doc.isSuccess()) {
             pdfURL = converter.getPDF(doc);

            // 输出生成的 PDF 和 HTML 链接。
            logger.info("转换生成的 PDF: " + pdfURL);
            logger.info("九云图转换完成。");
        } else {
     ...
     * pdf上传oss返回url
     */
    /*private String pdfUrlToOssUrl(String url){
